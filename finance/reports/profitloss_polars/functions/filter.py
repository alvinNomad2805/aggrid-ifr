...
import numpy as np
import polars as pl
from reports.profitloss_polars.functions import preprocessing
...
def applyfilter(companies, brands):
    ...
    if not isinstance(all_year, pl.DataFrame):
        all_year = pl.from_pandas(all_year)
        
    all_year = all_year.with_columns(pl.col('PeriodYear').cast(pl.String).alias('PeriodYear'))
    all_year = all_year.with_columns(pl.col('PeriodMonth').cast(pl.String).str.zfill(2).alias('PeriodMonth'))
    all_year = all_year.with_columns(pl.col('AccountNumber').cast(pl.String).alias('AccountNumber'))
    all_year = all_year.with_columns(pl.col('PeriodMonth').replace(old='13', new='12').alias('PeriodMonth'))

    if not isinstance(raw_data, pl.DataFrame):
        raw_data = pl.from_pandas(raw_data)

    raw_data = raw_data.filter(
        (pl.col('PeriodMonth') == selected_month) &
        (pl.col('PeriodYear') == str(select_year))
    )
    #end flask data
    ...
    print('HAAHAUHUWHUAHFUHEUAHFUA')
    print(all_year[all_year['Month'] == 'Unknown'])
