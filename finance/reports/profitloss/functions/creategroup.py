def create_group(raw_data):
    raw_data['AccountNumber2'] = raw_data['AccountNumber'].str[:2]
    ...
    raw_data.loc[:,'Group'] = raw_data['AccountNumber2'].map(lambda x: identify.get(x, ['Unknown', 'Unknown'])[0])
    raw_data.loc[:,'Group2'] = raw_data['AccountNumber2'].map(lambda x: identify.get(x, ['Unknown', 'Unknown'])[1])
